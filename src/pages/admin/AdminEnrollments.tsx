...
export default function AdminEnrollments() {
  ...
  const fetchEnrollments = async () => {
    ...
    setError("");
    try {
      // Backend: GET /enrollments/requests?status=PENDING
  ...
  const handleAction = async () => {
    ...
    try {
      // Backend: PUT /enrollments/requests/:id/approve or /reject
    ...
    <DashboardLayout>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{enrollments.length} request{enrollments.length !== 1 ? "s" : ""}</p>
        </div>
        <button onClick={fetchEnrollments} className="btn-outline-secondary flex items-center gap-2">
      ...
      {filtered.length === 0 ? (
        <div className="dash-card p-12 text-center">
          ...
          <p className="text-muted-foreground">No {filter === "ALL" ? "" : filter.toLowerCase()} enrollment requests</p>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map((enrollment) => (
                <tr key={enrollment.id}>
                  <td>
                    <p className="font-medium text-foreground">{enrollment.first_name} {enrollment.last_name}</p>
                    <p className="text-xs text-muted-foreground">{enrollment.email}</p>
                  </td>
                  <td className="text-foreground">{enrollment.course_title || `Course #${enrollment.course_id}`}</td>
                  <td className="text-foreground font-medium">{formatPrice(enrollment.amount || enrollment.course_price)}</td>
                  ...
                  <td className="text-muted-foreground">{formatDate(enrollment.requested_at)}</td>
                  {filter === "PENDING" && (
                    <td>
                      <div className="flex gap-2">
                        <button
                          onClick={() => { setActionId(enrollment.id); setActionType("approve"); }}
                        ...
                        <button
                          onClick={() => { setActionId(enrollment.id); setActionType("reject"); }}
                          disabled={processing === enrollment.id}
      ...
      {/* Image Viewer Modal */}
      {viewImage && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50" onClick={() => setViewImage(null)}>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-background rounded-xl max-w-md w-full p-6">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {actionType === "reject" ? "A rejection reason is required." : "Add a note (optional)"}
            </p>
            <textarea
              value={adminNotes}
              ...
              placeholder={actionType === "reject" ? "Reason for rejection (required)..." : "Admin notes..."}
              required={actionType === "reject"}
            />
            <div className="flex justify-end gap-3">
              <button onClick={() => { setActionId(null); setAdminNotes(""); }} className="btn-outline-secondary">Cancel</button>
              <button
                onClick={handleAction}
                disabled={processing === actionId || (actionType === "reject" && !adminNotes.trim())}
                className={actionType === "approve" ? "inline-flex items-center gap-1 px-4 py-2 text-sm rounded-lg text-white bg-emerald-600 hover:bg-emerald-700 transition-colors disabled:opacity-50 font-medium" : "btn-destructive disabled:opacity-50"}
              >
